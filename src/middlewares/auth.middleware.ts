...
export const registerMiddleware = async (req: Req, res: Res, next: Next) => {
  ...
  if (await userService.getUserByusername(body.username)) {
    return next(
      new BadrequestError(
        `${body.username} is already taken, choose another one`,
      ),
    );
  }
...
export const localAuthenticator = async (req: Req, res: Res, next: Next) => {
  authenticate(
    "local",
    { session: false },
...
export const jwtAuthenticator = async (req: Req, res: Res, next: Next) => {
...
export const loginMiddleware = async (req: Req, res: Res, next: Next) => {
